...
\begin{figure}[h]
	...
	\node[draw] at (-2,3) {Restriction};
	\node[draw] at (\w+2,0) {Prolongation};
	\draw [arrow] (-2,2.5) -- (-2, 0);
	\draw [arrow] (\w+2,0.5) -- (\w+2, 3);
	
	\foreach \x in {0,...,8}{
		\node at (\w/8*\x, 3) [circle,fill=black,scale=\scale] {};
	}
	\foreach \x in {0,...,4}{
		\node at (\w/4*\x, 1.5) [circle,fill=black,scale=\scale] {};
	}
	\foreach \x in {0,...,2}{
		\node at (\w/2*\x, 0) [circle,fill=black,scale=\scale] {};
	}
	\draw [arrow] (0,0) -- (\w, 0);
	\draw [arrow] (0,1.5) -- (\w, 1.5);
	\draw [arrow] (0,3) -- (\w, 3);
